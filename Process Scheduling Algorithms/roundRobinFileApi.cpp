...
void processTable(){
    cout<<setw(51)<<right<<"PROCESS TABLE\n";
...
void roundRobin(process arr[],int quantum,int n){
    ...
    ofstream outFile("roundRobinOutput.txt");
    if(outFile.is_open()){
        // Process Table
        outFile<<"ROUND ROBIN ALGORITHM WITH PROCESS TABLE AND GANTT CHART\n";
    ...
    else cout<<"Unable to open the file FinalStore.txt for writing.\n";
}
...
void displayTextFile(){
    ifstream inFile("roundRobinOutput.txt");
    if(inFile.is_open()){
        cout<<"\nDISPLAYING FROM THE TEXT FILE...\n\n";
        string line;
        while(getline(inFile, line)) cout<<line<<endl;
        inFile.close();
    }
    else cout<<"Unable to open the file roundRobinOuput.txt for reading.\n";
}
void menu(){
    int choice;
    while(true){
        cout<<"\n---------------------MENU---------------------\n";
        cout<<"Choose...\n";
        cout<<"1. Run Scheduler and store output in a TEXT FILE\n";
        cout<<"2. Display contents of TEXT FILE\n";
        cout<<"3. Exit\n";
        cin>>choice;
        switch(choice){
            case 1:
                cout<<"\nSCHEDULER RUNNING..."<<endl;
                initiator();
                break;
            case 2:
                displayTextFile();
