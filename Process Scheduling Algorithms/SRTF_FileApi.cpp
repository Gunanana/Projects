...
void shortestRemainingTimeFirst(process arr[],int n){
    ...
    // Writing to FinalStore.txt
    ofstream outFile("FinalStore.txt");
    if(outFile.is_open()){
        // Process Table
        outFile << "PROCESS TABLE\n";
        outFile << "| P No. | Arrival Time | Burst Time | Completion Time | Turn Around Time | Waiting Time |\n";
        for(auto i: processQ){
            outFile << "|  " << setw(3) << right << i->pno << "  |      " << setw(3) << right << i->at << "     |     " << setw(3) << right << i->bt << "    |       " << setw(3) << right << i->ct << "       |       " << setw(3) << right << i->tat << "        |      " << setw(3) << right << i->wt << "     |\n";
        }

        // Gantt Chart
        outFile << "\nGANTT CHART\n";
        for(auto it = gc.begin(); it != gc.end(); ++it) outFile << " <" << it->first.first << "> P" << it->second << " <" << it->first.second << "> |";
        
        // Average Turn Around time and Average Waiting Time
        outFile << "\n\nAverage Turn Around time: " << avgtat << " ms\n";
        outFile << "Average Waiting Time: " << avgwt << " ms\n";
        
        outFile.close();
    } else {
        cout << "Unable to open the file FinalStore.txt for writing.\n";
    }
}

// Initiator function to take input and run the scheduler
void initiator(){
    while(true){
        int num;
...
// Function to display contents of FinalStore.txt
void displayFinalStore(){
    ifstream inFile("FinalStore.txt");
    if(inFile.is_open()){
        string line;
        ...
        cout << "Unable to open the file FinalStore.txt for reading.\n";
    }
}
...
int main(){
    ...
    while(true){
        cout << "MENU:\n";
        cout << "1. Run scheduler and store output in FinalStore.txt\n";
        cout << "2. Display contents of FinalStore.txt\n";
        cout << "3. Exit\n";
        ...
        switch(choice){
            case 1:
            cout<<" "<<endl;
            cout<<"L E T ' S   G O ! ! !"<<endl;
                initiator();
                break;
            case 2:
                displayFinalStore();
                cout<<" "<<endl;
                cout<<"D I S P L A Y   S U C C E S S ! ! !"<<endl;
                cout<<" "<<endl;
                break;
            case 3:
                cout << "Exiting...\n";
                cout<<" "<<endl;
                cout<<"O K   D O N E ! ! !"<<endl;
                return 0;
            default:
